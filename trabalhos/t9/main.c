...
int main()
{
  ...
  printf("Digite o numero de strings q vc deseja ler: \n");
	...
	while(cont < numero_arqu){  /*laco para ler o numero de arquivos digitados*/
		if ((pFile = fopen (msg , "r") ) == NULL){   /*if para caso o arquivo esteja com o nome errado*/
			printf("Erro abrindo arquivo\n   %s  ",arquivos[0][cont]);
			exit(1);
		}
   		/* PARTE DO CODIGO QUE PERCORRE O ARQUIVO ACHA AS STRINGS , DEPOIS INSERE NA ARVORE AVL , logica feita com*/
		while(c = fgetc(pFile), c != EOF){
    	if(c != ' ' && c != '.' && c != ','  && c != ';' && c != '\t' && c != '\"' && c != ':' && c != '?' && c != '[' && c != ']' && c != '{' && c != '}' && c != '(' && c != ')' && c != '!'
   		...
   		|| c == '?' || c == '<' || c == '>' || c== '\n') && j ==1 ){
				...
				//fprintf(stderr,"inserindo - > %s \n",buff);
				//->send aux
				root = insert(root, buff,msg); /*insercao do buffer na arvore e do arquivo numa lista*/
			}
			if(c == '\n')j = 0;
		}
